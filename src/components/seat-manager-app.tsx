...
function AllocationView({ hall, session, onStats, revision }: { hall: Hall; session: Session; onStats: (distributed: number, entered: number) => void; revision: number }) {
  ...
  const existingGroups=[...Array.from(allocated.values()).reduce((groups,record)=>{
    if(record.allocation_status!=="distributed"||!record.group_name?.trim())return groups;
    const name=record.group_name.trim();
    const key=name.toLocaleLowerCase("ko-KR");
    const current=groups.get(key)??{name,count:0,entered:0,assigneeName:record.assignee_name??"",contact:record.contact??""};
    current.count+=1;
    if(record.admission_status==="entered")current.entered+=1;
    if(!current.assigneeName&&record.assignee_name)current.assigneeName=record.assignee_name;
    if(!current.contact&&record.contact)current.contact=record.contact;
    groups.set(key,current);
    return groups;
  },new Map<string,{name:string;count:number;entered:number;assigneeName:string;contact:string}>()).values()].sort((left,right)=>left.name.localeCompare(right.name,"ko-KR"));
  ...
  const chooseExistingGroup=(name:string)=>{
    const group=existingGroups.find((item)=>item.name===name);
    if(!group)return;
    changeGroupName(group.name);
    setAssigneeName(group.assigneeName);
  ...
  const startNewGroup=()=>{
    setGroupName("");
    setGroupColor(recommendedGroupColor);
    setAssigneeName("");
    setContact("");
    setPicked([]);
    setMessage(null);
  ...
  return <div className="view"><PageHeader eyebrow={`${hall.name} · ${session.event}`} title="좌석 배분" description={workflowMode==="allocate"?"개인 또는 단체에 실제 좌석을 배분합니다.":"배분된 좌석을 검색하고 일부 또는 전체를 취소합니다."} />
    ...
    {workflowMode==="allocate"&&recipientType==="group"&&<section className="existing-group-picker" aria-label="기존 단체 선택"><div><div><strong>기존 단체에 추가 배정</strong><span>단체를 선택하면 이름·색상·담당자 정보를 불러옵니다.</span></div><button type="button" onClick={startNewGroup}>새 단체 입력</button></div>{existingGroups.length?<div>{existingGroups.map((group)=>{const savedColor=groupColors.find((item)=>item.group_name.trim().toLocaleLowerCase("ko-KR")===group.name.toLocaleLowerCase("ko-KR"))?.color_hex??GROUP_COLORS[0].hex;const active=existingGroupColor?.group_name===group.name;return <button type="button" key={group.name} className={active?"active":""} onClick={()=>chooseExistingGroup(group.name)}><i style={{background:savedColor}}/><span><strong>{group.name}</strong><small>{group.count}석 · 입장 {group.entered}석</small></span></button>})}</div>:<p>아직 배정된 단체가 없습니다. 아래에서 새 단체를 입력하세요.</p>}</section>}
...
function EntryView({ hall, session, onStats, revision }: { hall: Hall; session: Session; onStats: (distributed: number, entered: number) => void; revision: number }) {
  ...
  const submit=async()=>{if(!user){requestAuth();return;}if(!picked.length){setMessage({tone:"error",text:mode==="admit"?"입장 처리할 좌석을 선택해 주세요.":"입장을 취소할 좌석을 선택해 주세요."});return;}if(mode==="undo"&&!window.confirm(`선택한 ${picked.length}석의 입장 처리를 취소할까요?\n취소 이력은 감사 로그에 보존됩니다.`))return;setBusy(true);setMessage(null);const client=createClient();const response=mode==="admit"?await client.rpc("operate_session_admissions",{p_session_code:session.id,p_seat_ids:picked,p_mode:mode,p_entrance_name:entrance.trim()||"정문",p_reason:reason.trim()||"입장 관리 화면에서 취소"}):await client.rpc("undo_session_admissions",{p_session_code:session.id,p_seat_ids:picked,p_entrance_name:entrance.trim()||"정문",p_reason:reason.trim()||"입장 관리 화면에서 취소"});if(response.error){setMessage({tone:"error",text:response.error.message});setBusy(false);return;}const count=Number(response.data??picked.length);setPicked([]);setReason("");setMessage({tone:"success",text:mode==="admit"?`${count}석의 입장이 완료되었습니다.`:`${count}석의 입장이 취소되고 입장 전 배분 상태로 복원되었습니다.`});await load();setBusy(false);};
  ...
  return <div className="view view--entry"><PageHeader eyebrow={`${hall.name} · ${session.event}`} title="입장 관리" description="좌석번호·이름·단체명으로 검색하고, 좌석별 또는 단체 전체를 한 번에 처리합니다." />
...
type HistoryFilter="all"|"entry"|"allocation"|"cancel"|"reassign"|"block";
type HistoryState=Record<string,unknown>;
type HistoryItem={
  log_id:number;session_seat_id:number|null;seat_id:number|null;action:string;reason:string|null;entrance_name:string|null;
  before_state:HistoryState;after_state:HistoryState;created_at:string;actor_name:string;seat_code:string|null;floor_code:string|null;
  floor_name:string|null;row_label:string|null;seat_number:number|null;current_allocation_status:string|null;current_admission_status:string|null;
  can_undo:boolean;total_count:number;
};
...
function HistoryView({ hall,session,revision }: { hall:Hall;session:Session;revision:number }) {
  const {user,requestAuth}=useCatalog();
  ...
  const load=async(offset=0)=>{if(!user)return;if(offset)setLoadingMore(true);else setLoading(true);const {data,error}=await createClient().rpc("list_session_audit_logs",{p_session_code:session.id,p_action_filter:filter,p_query:appliedQuery,p_offset:offset,p_limit:50});if(error){setMessage({tone:"error",text:error.message});}else{const next=(data??[]) as HistoryItem[];setItems((current)=>offset?[...current,...next]:next);setTotal(Number(next[0]?.total_count??(offset?total:0)));if(!offset)setSelected((current)=>current&&next.some((item)=>item.log_id===current.log_id)?next.find((item)=>item.log_id===current.log_id)??null:null);}setLoading(false);setLoadingMore(false);};
  useEffect(()=>{let active=true;if(!user)return()=>{active=false;};void createClient().rpc("list_session_audit_logs",{p_session_code:session.id,p_action_filter:filter,p_query:appliedQuery,p_offset:0,p_limit:50}).then(({data,error})=>{if(!active)return;if(error){setMessage({tone:"error",text:error.message});setItems([]);setTotal(0);}else{const next=(data??[]) as HistoryItem[];setItems(next);setTotal(Number(next[0]?.total_count??0));setSelected((current)=>current&&next.some((item)=>item.log_id===current.log_id)?next.find((item)=>item.log_id===current.log_id)??null:null);}setLoading(false);});return()=>{active=false;};},[user,session.id,filter,appliedQuery,revision,refreshKey]);
  const undo=async(item:HistoryItem)=>{if(!user){requestAuth();return;}if(!item.can_undo||!item.seat_id)return;if(!window.confirm(`${item.floor_name??""} ${item.row_label??""}-${String(item.seat_number??"").padStart(2,"0")} 좌석의 입장을 취소할까요?\n기존 입장 기록은 삭제되지 않습니다.`))return;setBusy(true);setMessage(null);const {error}=await createClient().rpc("undo_session_admissions",{p_session_code:session.id,p_seat_ids:[item.seat_id],p_entrance_name:"처리 이력",p_reason:"처리 이력에서 최근 입장 취소"});if(error)setMessage({tone:"error",text:error.message});else{setMessage({tone:"success",text:"입장을 취소하고 입장 전 좌석 상태로 복원했습니다."});setSelected(null);setRefreshKey((value)=>value+1);}setBusy(false);};
  const targetLabel=(item:HistoryItem)=>item.floor_name&&item.row_label&&item.seat_number!=null?`${item.floor_name} ${item.row_label}-${String(item.seat_number).padStart(2,"0")}`:item.seat_code??"회차 전체";
  return <div className="view view--history"><PageHeader eyebrow={`${hall.name} 감사 로그`} title="처리 이력" description="현재 행사 회차의 입장·배분·취소·재배정 기록을 실시간으로 확인합니다." />
    {!user?<section className="content-card empty-state auth-empty-state"><span className="empty-state-mark" aria-hidden="true">이력</span><strong>처리 이력을 확인하려면 로그인하세요</strong><p>로그인하면 현재 회차의 좌석 배분·입장·취소·재배정 기록을 시간순으로 확인할 수 있습니다.</p><button type="button" className="primary-button" onClick={requestAuth}>직원 로그인</button></section>:<>
      {message&&<p className={`allocation-message allocation-message--${message.tone}`} role="status">{message.text}</p>}
      <section className="history-controls content-card"><div className="filter-chips">{HISTORY_FILTERS.map(([id,label])=><button type="button" className={filter===id?"active":""} key={id} onClick={()=>{setLoading(true);setFilter(id);setSelected(null);setMessage(null);}}>{label}</button>)}</div><form className="history-search" onSubmit={(event)=>{event.preventDefault();setLoading(true);setAppliedQuery(query.trim());setSelected(null);}}><label htmlFor="history-query">이력 검색</label><input id="history-query" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="좌석·담당자·단체·사유"/><button type="submit">검색</button>{appliedQuery&&<button type="button" className="history-search-reset" onClick={()=>{setLoading(true);setQuery("");setAppliedQuery("");}}>초기화</button>}</form><div className="history-count"><strong>{total}</strong><span>건</span></div></section>
      <section className="history-layout"><article className="content-card history-list-card"><div className="section-title"><div><h2>회차 처리 내역</h2><p>{session.date} {session.time} · {session.event}</p></div><small>최신순</small></div>{loading?<div className="compact-empty">처리 이력을 불러오는 중입니다.</div>:!items.length?<div className="empty-state history-empty"><strong>조건에 맞는 처리 이력이 없습니다</strong><p>좌석 배정이나 입장 처리가 완료되면 이곳에 자동으로 기록됩니다.</p></div>:<div className="history-table"><div className="history-head"><span>시간</span><span>담당자</span><span>대상</span><span>처리</span><span>상태</span></div>{items.map((item)=><button type="button" className={`history-row ${selected?.log_id===item.log_id?"active":""}`} key={item.log_id} onClick={()=>setSelected(item)}><time>{historyTime(item.created_at)}</time><strong>{item.actor_name}</strong><span>{targetLabel(item)}</span><span>{HISTORY_ACTION_LABEL[item.action]??item.action}</span><em>{item.can_undo?"취소 가능":"기록 완료"}</em></button>)}</div>}{items.length<total&&<button className="history-more secondary-button" disabled={loadingMore} onClick={()=>void load(items.length)}>{loadingMore?"불러오는 중…":`이전 기록 더 보기 (${items.length}/${total})`}</button>}</article>
        <aside className="content-card history-detail">{selected?<><div className="history-detail-heading"><div><span>{HISTORY_ACTION_LABEL[selected.action]??selected.action}</span><h2>{targetLabel(selected)}</h2><p>{historyTime(selected.created_at)}</p></div><button type="button" onClick={()=>setSelected(null)}>닫기</button></div><dl><div><dt>담당자</dt><dd>{selected.actor_name}</dd></div><div><dt>처리 위치</dt><dd>{selected.entrance_name||"기록 없음"}</dd></div><div><dt>변경 전</dt><dd>{historyStateLabel(selected.before_state)}</dd></div><div><dt>변경 후</dt><dd>{historyStateLabel(selected.after_state)}</dd></div>{(historyPerson(selected.after_state)||historyPerson(selected.before_state))&&<div><dt>배부 대상</dt><dd>{historyPerson(selected.after_state)||historyPerson(selected.before_state)}</dd></div>}<div className="history-reason"><dt>처리 사유</dt><dd>{selected.reason||"기록 없음"}</dd></div></dl><p className="history-integrity-note">이 기록은 수정하거나 삭제하지 않습니다. 취소 작업도 새로운 이력으로 보존됩니다.</p>{selected.can_undo&&<button className="danger-button" disabled={busy} onClick={()=>void undo(selected)}>{busy?"취소 처리 중…":"이 입장 바로 취소"}</button>}</>:<div className="empty-state history-detail-empty"><strong>기록 상세보기</strong><p>왼쪽 처리 내역을 선택하면 변경 전후 상태와 담당자·사유를 확인할 수 있습니다.</p></div>}</aside>
      </section>
    </>}
  </div>;
}
...
function SeatMapView({ floor,setFloor,query,setQuery,selected,setSelected,setConfirmed,hall,session,onStats,revision,realtimeStatus }: SeatMapViewProps) {
  ...
  const mapDragRef=useRef<{pointerId:number;startX:number;startY:number;scrollLeft:number;scrollTop:number;moved:boolean}|null>(null);
  const mapPointersRef=useRef(new Map<number,{x:number;y:number}>());
  const mapPinchRef=useRef<{startDistance:number;startScale:number;contentX:number;contentY:number;moved:boolean}|null>(null);
  const mapScaleRef=useRef(1);
  const suppressSeatClickRef=useRef(0);
  ...
  const handleMapSeat=(seat:Seat)=>{if(Date.now()<suppressSeatClickRef.current)return;const state=resolveSeatState(seat.row,seat.number);setSelected(seat);setConfirmed(false);setMessage(null);if(quickAdmission&&!admissionBusy&&["empty","distributed","onsite"].includes(state.status))void admitSeat(seat,state);};
  const startMapDrag=(event:PointerEvent<HTMLElement>)=>{
    if(event.pointerType==="mouse"&&event.button!==0)return;
    const panel=event.currentTarget;mapPointersRef.current.set(event.pointerId,{x:event.clientX,y:event.clientY});
    ...
    if(pointers.length>=2){const [first,second]=pointers;const rect=panel.getBoundingClientRect();const centerX=(first.x+second.x)/2-rect.left;const centerY=(first.y+second.y)/2-rect.top;const startScale=mapScaleRef.current;mapPinchRef.current={startDistance:Math.hypot(second.x-first.x,second.y-first.y),startScale,contentX:(panel.scrollLeft+centerX)/startScale,contentY:(panel.scrollTop+centerY)/startScale,moved:false};mapDragRef.current=null;setMapDragging(true);event.preventDefault();return;}
    if(mapScaleRef.current<=1)return;
    mapDragRef.current={pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,scrollLeft:panel.scrollLeft,scrollTop:panel.scrollTop,moved:false};setMapDragging(true);
  };
  const moveMapDrag=(event:PointerEvent<HTMLElement>)=>{
    ...
    const panel=event.currentTarget;const pointers=[...mapPointersRef.current.values()];const pinch=mapPinchRef.current;
    if(pinch&&pointers.length>=2){const [first,second]=pointers;const distance=Math.hypot(second.x-first.x,second.y-first.y);if(pinch.startDistance<=0)return;const nextScale=Math.min(2.5,Math.max(1,pinch.startScale*(distance/pinch.startDistance)));const rect=panel.getBoundingClientRect();const centerX=(first.x+second.x)/2-rect.left;const centerY=(first.y+second.y)/2-rect.top;pinch.moved=pinch.moved||Math.abs(nextScale-pinch.startScale)>.01;mapScaleRef.current=nextScale;setMapScale(nextScale);requestAnimationFrame(()=>{panel.scrollLeft=pinch.contentX*nextScale-centerX;panel.scrollTop=pinch.contentY*nextScale-centerY;});event.preventDefault();return;}
    const drag=mapDragRef.current;if(!drag||drag.pointerId!==event.pointerId)return;const deltaX=event.clientX-drag.startX;const deltaY=event.clientY-drag.startY;if(Math.abs(deltaX)>3||Math.abs(deltaY)>3)drag.moved=true;panel.scrollLeft=drag.scrollLeft-deltaX;panel.scrollTop=drag.scrollTop-deltaY;if(event.pointerType!=="mouse")event.preventDefault();
  };
  const stopMapDrag=(event:PointerEvent<HTMLElement>)=>{
    const panel=event.currentTarget;const drag=mapDragRef.current;const pinch=mapPinchRef.current;mapPointersRef.current.delete(event.pointerId);
    if(drag?.moved||pinch?.moved)suppressSeatClickRef.current=Date.now()+450;
    if(panel.hasPointerCapture(event.pointerId))panel.releasePointerCapture(event.pointerId);
    const remaining=[...mapPointersRef.current.entries()];
    if(pinch&&remaining.length===1&&mapScaleRef.current>1){const [pointerId,point]=remaining[0];mapPinchRef.current=null;mapDragRef.current={pointerId,startX:point.x,startY:point.y,scrollLeft:panel.scrollLeft,scrollTop:panel.scrollTop,moved:true};return;}
    if(!remaining.length){mapPinchRef.current=null;mapDragRef.current=null;setMapDragging(false);}
  };
...
function SeatManagerAppInner() {
  ...
  useEffect(()=>{workspaceContentRef.current?.scrollTo({top:0,left:0});},[active,session?.sessionId]);
