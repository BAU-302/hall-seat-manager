...
function EntryView() {
  return <div className="view"><PageHeader eyebrow="입장 운영" title="입장 관리" description="좌석번호나 배부 대상을 검색해 빠르게 입장 처리합니다." />
    <section className="split-layout split-layout--entry"><article className="content-card form-card"><div className="section-title"><div><h2>좌석 빠른 검색</h2><p>C-12, 1 C 12, 이름 또는 단체명</p></div></div><div className="big-search"><input placeholder="좌석 또는 이름 검색" aria-label="입장 좌석 검색"/><button disabled>검색</button></div><div className="empty-state"><strong>입장 데이터가 없습니다</strong><p>행사 등록 후 좌석 배분 기능을 연결하면 실제 검색 결과가 표시됩니다.</p></div></article>
      <article className="content-card"><div className="section-title"><div><h2>최근 처리</h2><p>실수 시 즉시 취소할 수 있습니다.</p></div></div><div className="compact-empty">아직 처리된 입장 기록이 없습니다.</div></article></section>
  </div>;
}

function ScanView() {
  return <div className="view"><PageHeader eyebrow="모바일 우선 기능" title="QR 스캔" description="QR은 좌석표와 분리된 전용 화면에서 빠르게 처리합니다." />
    <section className="scan-layout"><article className="scanner-card"><div className="camera-preview"><div className="scan-corners"><span/><span/><span/><span/></div><p>카메라 연결 준비 중</p></div><button className="primary-button" disabled>QR 카메라 시작</button><p className="helper-text">행사·좌석 데이터 연결 후 실제 모바일 카메라를 활성화합니다.</p></article><article className="content-card scan-result"><div className="section-title"><div><h2>스캔 결과</h2><p>처리 전 배부 대상과 좌석을 확인합니다.</p></div></div><div className="empty-state"><strong>입장 데이터가 없습니다</strong><p>QR을 인식하면 이곳에 실제 좌석 정보가 표시됩니다.</p></div></article></section>
  </div>;
}
...
function SeatMapView({ floor,setFloor,query,setQuery,selected,setSelected,setConfirmed,hall }: SeatMapViewProps) {
