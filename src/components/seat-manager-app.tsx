...
function SeatMapView({ floor,setFloor,query,setQuery,selected,setSelected,setConfirmed,hall,session,onStats,revision,realtimeStatus }: SeatMapViewProps) {
  ...
  const handleMapSeat=(seat:Seat)=>{if(suppressSeatClickRef.current){suppressSeatClickRef.current=false;return;}const state=resolveSeatState(seat.row,seat.number);setSelected(seat);setConfirmed(false);setMessage(null);if(quickAdmission&&!admissionBusy&&["empty","distributed","onsite"].includes(state.status))void admitSeat(seat,state);};
  const startMapDrag=(event:PointerEvent<HTMLElement>)=>{if(event.button!==0||mapScale<=1)return;const panel=event.currentTarget;mapDragRef.current={pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,scrollLeft:panel.scrollLeft,scrollTop:panel.scrollTop,moved:false};panel.setPointerCapture(event.pointerId);setMapDragging(true);};
  const moveMapDrag=(event:PointerEvent<HTMLElement>)=>{const drag=mapDragRef.current;if(!drag||drag.pointerId!==event.pointerId)return;const deltaX=event.clientX-drag.startX;const deltaY=event.clientY-drag.startY;if(Math.abs(deltaX)>3||Math.abs(deltaY)>3)drag.moved=true;event.currentTarget.scrollLeft=drag.scrollLeft-deltaX;event.currentTarget.scrollTop=drag.scrollTop-deltaY;};
  const stopMapDrag=(event:PointerEvent<HTMLElement>)=>{const drag=mapDragRef.current;if(!drag||drag.pointerId!==event.pointerId)return;suppressSeatClickRef.current=drag.moved;mapDragRef.current=null;if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);setMapDragging(false);};
  ...
  return <div className="view view--seats"><PageHeader eyebrow="실시간 좌석 운영" title={`${floor} 좌석 현황`} description={!user?"직원 로그인 후 실제 배분 상태를 확인할 수 있습니다.":stateLoading?"배분 상태를 불러오는 중입니다.":`${session.event} 배분 상태가 반영되었습니다.`} />
    ...
    <section className={`map-panel map-panel--${floor==='1층'?'one':'two'} ${mapScale>1?'is-zoomed':''} ${mapDragging?'is-dragging':''}`} onPointerDown={startMapDrag} onPointerMove={moveMapDrag} onPointerUp={stopMapDrag} onPointerCancel={stopMapDrag} onWheel={(event)=>{if(!event.ctrlKey&&!event.metaKey)return;event.preventDefault();setMapScale((value)=>Math.min(2.5,Math.max(1,value+(event.deltaY<0?.15:-.15))));}}><div className="seat-map-canvas" style={{width:`${mapScale*100}%`,height:`${mapScale*100}%`}}>{floor==='1층'?<FirstFloorSvg selectedId={selected?.id} queryId={queryId} seatState={resolveSeatState} onSelect={handleMapSeat}/>:<SecondFloorSvg selectedId={selected?.id} queryId={queryId} seatState={resolveSeatState} onSelect={handleMapSeat}/>}</div></section>
